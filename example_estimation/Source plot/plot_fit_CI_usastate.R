...
# plot Minnesota
# load data
Observed<-read.xlsx("Source Data/plot_fit_CI_usastate/mn.xlsx", sheetName = "bothtwonpis")
hatyci<-read.xlsx("Source Data/plot_fit_CI_usastate/mn.xlsx", sheetName = "hatyci")
lrci<-read.xlsx("Source Data/plot_fit_CI_usastate/mn.xlsx", sheetName = "lrci")
...
plot_cn_fit <-ggplot()+
  ...
  scale_shape_manual(values = c(0, 1), 
                     name = "Treatment")+
  annotate("text", label = "Minnesota", x = 25, y =  0.59 * scale, color = "#8B658B",fontface = 'bold')+
  ...
  geom_segment(aes(x = 1, xend = 511, y = -Inf, yend = -Inf), color = "darkgray") +
...
# plot Colorado
# load data
Observed<-read.xlsx("Source Data/plot_fit_CI_usastate/co.xlsx", sheetName = "bothtwonpis")
hatyci<-read.xlsx("Source Data/plot_fit_CI_usastate/co.xlsx", sheetName = "hatyci")
lrci<-read.xlsx("Source Data/plot_fit_CI_usastate/co.xlsx", sheetName = "lrci")

csobserved<-Observed%>% filter(time>=201140&time<=202128)%>%
  mutate(scenario = "Observed", mean = positive_rate,lower = positive_rate,upper = positive_rate)
tmp<-csobserved
tmp[tmp$time>=201240&tmp$time<=202028,c("mean","lower","upper")]<-hatyci[hatyci$time>=201240&hatyci$time<=202028,c("mean","lower","upper")]
tmp[tmp$time>=202029&tmp$time<=202128,c("mean","lower","upper")]<-lrci[lrci$time>=202029&lrci$time<=202128,c("mean","lower","upper")]
...
plot_cs_fit <-ggplot()+
  ...
  scale_shape_manual(values = c(0, 1), 
                     name = "Treatment")+
  annotate("text", label = "Colorado", x = 25, y =  0.59 * scale, color = "#8B658B",fontface = 'bold')+
...
# plot Indiana
# load data
Observed<-read.xlsx("Source Data/plot_fit_CI_usastate/in.xlsx", sheetName = "bothtwonpis")
hatyci<-read.xlsx("Source Data/plot_fit_CI_usastate/in.xlsx", sheetName = "hatyci")
lrci<-read.xlsx("Source Data/plot_fit_CI_usastate/in.xlsx", sheetName = "lrci")

ukobserved<-Observed%>% filter(time>=201140&time<=202128)%>%
  mutate(scenario = "Observed", mean = positive_rate,lower = positive_rate,upper = positive_rate)
tmp<-ukobserved
tmp[tmp$time>=201240&tmp$time<=202030,c("mean","lower","upper")]<-hatyci[hatyci$time>=201240&hatyci$time<=202030,c("mean","lower","upper")]
tmp[tmp$time>=202031&tmp$time<=202128,c("mean","lower","upper")]<-lrci[lrci$time>=202031&lrci$time<=202128,c("mean","lower","upper")]
...
plot_uk_fit <-ggplot()+
  ...
  scale_shape_manual(values = c(0, 1), 
                     name = "Treatment")+
  annotate("text", label = "Indiana", x = 25, y =  0.59 * scale, color = "#8B658B",fontface = 'bold')+
...
# plot Washington
# load data
Observed<-read.xlsx("Source Data/plot_fit_CI_usastate/wa.xlsx", sheetName = "bothtwonpis")
hatyci<-read.xlsx("Source Data/plot_fit_CI_usastate/wa.xlsx", sheetName = "hatyci")
lrci<-read.xlsx("Source Data/plot_fit_CI_usastate/wa.xlsx", sheetName = "lrci")

usaobserved<-Observed%>% filter(time>=201140&time<=202128)%>%
  mutate(scenario = "Observed", mean = positive_rate,lower = positive_rate,upper = positive_rate)
tmp<-usaobserved
tmp[tmp$time>=201240&tmp$time<=202023,c("mean","lower","upper")]<-hatyci[hatyci$time>=201240&hatyci$time<=202023,c("mean","lower","upper")]
tmp[tmp$time>=202024&tmp$time<=202128,c("mean","lower","upper")]<-lrci[lrci$time>=202024&lrci$time<=202128,c("mean","lower","upper")]
...
plot_usa_fit <-ggplot()+
  ...
  scale_shape_manual(values = c(0, 1), 
                     name = "Treatment")+
  annotate("text", label = "Washington", x = 25, y =  0.59 * scale, color = "#8B658B",fontface = 'bold')+
...
prow <- plot_grid(plot_cs_fit + labs(x = ""),
                  plot_uk_fit + theme(legend.position = "none") + labs(x = ""),
                  plot_cn_fit + theme(legend.position = "none") + labs(x = ""),
...
outfile <- glue("Source plot/output/Fit_2011-2021_CI_usastate.tif")
tiff(outfile, width = 20, height = 12, unit = "in", res = 300, compression = "lzw")
# pdf("Source plot/output/Fit_2011-2021_CI_usastate.pdf",width = 20, height = 12)
print(finalplot)
dev.off()
