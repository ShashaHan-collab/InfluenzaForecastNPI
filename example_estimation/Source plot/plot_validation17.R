...
# plot northern china
# load data
Observed<-read.xlsx("Source Data/plot_validation17/cn.xlsx", sheetName = "bothtwonpis")
hatyci<-read.xlsx("Source Data/plot_validation17/cn.xlsx", sheetName = "val17")


cnobserved<-Observed%>% filter(time>=201140&time<=201801)%>%
  mutate(positive_rate = replace(positive_rate, time > 201752, NA))%>%
  mutate(scenario = "Observed", mean = positive_rate,lower = positive_rate,upper = positive_rate)
tmp<-cnobserved
tmp[tmp$time>=201240&tmp$time<=201801,c("mean","lower","upper")]<-hatyci[hatyci$time>=201240&hatyci$time<=201801,c("mean","lower","upper")]
cnfit<-tmp%>% filter(time>=201140&time<=201801)%>%
  ...
  mutate(mean = replace(mean, time > 201752, NA))%>%
  mutate(lower = replace(lower, time > 201752, NA))%>%
  mutate(upper = replace(upper, time > 201752, NA))%>%
...
plot_cn_fit <-ggplot()+
  ...
  scale_x_continuous(breaks=cnxlabelpos, label=cnxlabel,expand = c(0.01,0),sec.axis=dup_axis(name="",breaks = c(1,274),labels = c("","")))+
  scale_y_continuous(limits = c(0, ylimits), breaks = seq(0, ylimits, by = 0.1* scale)) +
  theme_classic() +
  theme(axis.line = element_blank()) +
  geom_segment(aes(x = 1, xend = 326, y = -Inf, yend = -Inf), color = "darkgray") +
...
# plot Southern China
# load data
Observed<-read.xlsx("Source Data/plot_validation17/cs.xlsx", sheetName = "bothtwonpis")
hatyci<-read.xlsx("Source Data/plot_validation17/cs.xlsx", sheetName = "val17")

csobserved<-Observed%>% filter(time>=201140&time<=201801)%>%
  mutate(positive_rate = replace(positive_rate, time > 201752, NA))%>%
  mutate(scenario = "Observed", mean = positive_rate,lower = positive_rate,upper = positive_rate)
tmp<-csobserved
tmp[tmp$time>=201240&tmp$time<=201801,c("mean","lower","upper")]<-hatyci[hatyci$time>=201240&hatyci$time<=201801,c("mean","lower","upper")]
csfit<-tmp%>% filter(time>=201140&time<=201801)%>%
  mutate(mean = replace(mean, time < 201240, NA))%>%
  mutate(lower = replace(lower, time < 201240, NA))%>%
  mutate(upper = replace(upper, time < 201240, NA))%>%
  mutate(mean = replace(mean, time > 201752, NA))%>%
...
plot_cs_fit <-ggplot()+
  ...
  labs(x="year", y = "Percent Positive (%)") +
  scale_x_continuous(breaks=csxlabelpos, label=csxlabel,expand = c(0.01,0),sec.axis=dup_axis(name="",breaks = c(1,274),labels = c("","")))+
  scale_y_continuous(limits = c(0, ylimits), breaks = seq(0, ylimits, by = 0.1* scale)) +
  theme_classic() +
  theme(axis.line = element_blank()) +
  geom_segment(aes(x = 1, xend = 326, y = -Inf, yend = -Inf), color = "darkgray") +
...
# plot England
# load data
Observed<-read.xlsx("Source Data/plot_validation17/uk.xlsx", sheetName = "bothtwonpis")
hatyci<-read.xlsx("Source Data/plot_validation17/uk.xlsx", sheetName = "val17")

ukobserved<-Observed%>% filter(time>=201140&time<=201801)%>%
  mutate(positive_rate = replace(positive_rate, time > 201752, NA))%>%
  mutate(scenario = "Observed", mean = positive_rate,lower = positive_rate,upper = positive_rate)
tmp<-ukobserved
tmp[tmp$time>=201240&tmp$time<=201801,c("mean","lower","upper")]<-hatyci[hatyci$time>=201240&hatyci$time<=201801,c("mean","lower","upper")]
ukfit<-tmp%>% filter(time>=201140&time<=201801)%>%
  mutate(mean = replace(mean, time < 201240, NA))%>%
  mutate(lower = replace(lower, time < 201240, NA))%>%
  mutate(upper = replace(upper, time < 201240, NA))%>%
  mutate(mean = replace(mean, time > 201752, NA))%>%
...
plot_uk_fit <-ggplot()+
  ...
  labs(x="year", y = "Percent Positive (%)") +
  scale_x_continuous(breaks=ukxlabelpos, label=ukxlabel,expand = c(0.01,0),sec.axis=dup_axis(name="",breaks = c(1,275),labels = c("","")))+
  scale_y_continuous(limits = c(0, ylimits), breaks = seq(0, ylimits, by = 0.1* scale)) +
  theme_classic() +
  theme(axis.line = element_blank()) +
  geom_segment(aes(x = 1, xend = 326, y = -Inf, yend = -Inf), color = "darkgray") +
...
# plot The U.S.
# load data
Observed<-read.xlsx("Source Data/plot_validation17/usa.xlsx", sheetName = "bothtwonpis")
hatyci<-read.xlsx("Source Data/plot_validation17/usa.xlsx", sheetName = "val17")

usaobserved<-Observed%>% filter(time>=201140&time<=201801)%>%
  mutate(positive_rate = replace(positive_rate, time > 201752, NA))%>%
  mutate(scenario = "Observed", mean = positive_rate,lower = positive_rate,upper = positive_rate)
tmp<-usaobserved
tmp[tmp$time>=201240&tmp$time<=201801,c("mean","lower","upper")]<-hatyci[hatyci$time>=201240&hatyci$time<=201801,c("mean","lower","upper")]
usafit<-tmp%>% filter(time>=201140&time<=201801)%>%
  mutate(mean = replace(mean, time < 201240, NA))%>%
  mutate(lower = replace(lower, time < 201240, NA))%>%
  mutate(upper = replace(upper, time < 201240, NA))%>%
  mutate(mean = replace(mean, time > 201752, NA))%>%
...
plot_usa_fit <-ggplot()+
  ...
  labs(x="year", y = "Percent Positive (%)") +
  scale_x_continuous(breaks=usaxlabelpos, label=usaxlabel,expand = c(0.01,0),sec.axis=dup_axis(name="",breaks = c(1,275),labels = c("","")))+
  scale_y_continuous(limits = c(0, ylimits), breaks = seq(0, ylimits, by = 0.1* scale)) +
  theme_classic() +
  theme(axis.line = element_blank()) +
  geom_segment(aes(x = 1, xend = 326, y = -Inf, yend = -Inf), color = "darkgray") +
...
title <- ggdraw()+
  ...
  draw_label("Test", hjust = -22, size=13,vjust = 2.3)
...
outfile <- glue("Source plot/output/Validation_2011-2017.tif")
