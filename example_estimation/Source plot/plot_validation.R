...
# plot northern china
# load data
Observed<-read.xlsx("Source Data/plot_validation/cn.xlsx", sheetName = "bothtwonpis")
hatyci<-read.xlsx("Source Data/plot_validation/cn.xlsx", sheetName = "val")


cnobserved<-Observed%>% filter(time>=201140&time<=202001)%>%
  mutate(positive_rate = replace(positive_rate, time > 201952, NA))%>%
  mutate(scenario = "Observed", mean = positive_rate,lower = positive_rate,upper = positive_rate)
tmp<-cnobserved
tmp[tmp$time>=201240&tmp$time<=202001,c("mean","lower","upper")]<-hatyci[hatyci$time>=201240&hatyci$time<=202001,c("mean","lower","upper")]
cnfit<-tmp%>% filter(time>=201140&time<=202001)%>%
  ...
  mutate(mean = replace(mean, time > 201952, NA))%>%
  mutate(lower = replace(lower, time > 201952, NA))%>%
  mutate(upper = replace(upper, time > 201952, NA))%>%
...
plot_cn_fit <-ggplot()+
  ...
  scale_x_continuous(breaks=cnxlabelpos, label=cnxlabel,expand = c(0.01,0),sec.axis=dup_axis(name="",breaks = c(1,378),labels = c("","")))+
  scale_y_continuous(limits = c(0, ylimits), breaks = seq(0, ylimits, by = 0.1* scale)) +
  theme_classic() +
  theme(axis.line = element_blank()) +
  geom_segment(aes(x = 1, xend = 430, y = -Inf, yend = -Inf), color = "darkgray") +
...
# plot Southern China
# load data
Observed<-read.xlsx("Source Data/plot_validation/cs.xlsx", sheetName = "bothtwonpis")
hatyci<-read.xlsx("Source Data/plot_validation/cs.xlsx", sheetName = "val")

csobserved<-Observed%>% filter(time>=201140&time<=202001)%>%
  mutate(positive_rate = replace(positive_rate, time > 201952, NA))%>%
  mutate(scenario = "Observed", mean = positive_rate,lower = positive_rate,upper = positive_rate)
tmp<-csobserved
tmp[tmp$time>=201240&tmp$time<=202001,c("mean","lower","upper")]<-hatyci[hatyci$time>=201240&hatyci$time<=202001,c("mean","lower","upper")]
csfit<-tmp%>% filter(time>=201140&time<=202001)%>%
  mutate(mean = replace(mean, time < 201240, NA))%>%
  mutate(lower = replace(lower, time < 201240, NA))%>%
  mutate(upper = replace(upper, time < 201240, NA))%>%
  mutate(mean = replace(mean, time > 201952, NA))%>%
...
plot_cs_fit <-ggplot()+
  ...
  labs(x="year", y = "Percent Positive (%)") +
  scale_x_continuous(breaks=csxlabelpos, label=csxlabel,expand = c(0.01,0),sec.axis=dup_axis(name="",breaks = c(1,378),labels = c("","")))+
  scale_y_continuous(limits = c(0, ylimits), breaks = seq(0, ylimits, by = 0.1* scale)) +
  theme_classic() +
  theme(axis.line = element_blank()) +
  geom_segment(aes(x = 1, xend = 430, y = -Inf, yend = -Inf), color = "darkgray") +
...
# plot England
# load data
Observed<-read.xlsx("Source Data/plot_validation/uk.xlsx", sheetName = "bothtwonpis")
hatyci<-read.xlsx("Source Data/plot_validation/uk.xlsx", sheetName = "val")

ukobserved<-Observed%>% filter(time>=201140&time<=202001)%>%
  mutate(positive_rate = replace(positive_rate, time > 201952, NA))%>%
  mutate(scenario = "Observed", mean = positive_rate,lower = positive_rate,upper = positive_rate)
tmp<-ukobserved
tmp[tmp$time>=201240&tmp$time<=202001,c("mean","lower","upper")]<-hatyci[hatyci$time>=201240&hatyci$time<=202001,c("mean","lower","upper")]
ukfit<-tmp%>% filter(time>=201140&time<=202001)%>%
  mutate(mean = replace(mean, time < 201240, NA))%>%
  mutate(lower = replace(lower, time < 201240, NA))%>%
  mutate(upper = replace(upper, time < 201240, NA))%>%
  mutate(mean = replace(mean, time > 201952, NA))%>%
...
plot_uk_fit <-ggplot()+
  ...
  labs(x="year", y = "Percent Positive (%)") +
  scale_x_continuous(breaks=ukxlabelpos, label=ukxlabel,expand = c(0.01,0),sec.axis=dup_axis(name="",breaks = c(1,379),labels = c("","")))+
  scale_y_continuous(limits = c(0, ylimits), breaks = seq(0, ylimits, by = 0.1* scale)) +
  theme_classic() +
  theme(axis.line = element_blank()) +
  geom_segment(aes(x = 1, xend = 430, y = -Inf, yend = -Inf), color = "darkgray") +
...
# plot The U.S.
# load data
Observed<-read.xlsx("Source Data/plot_validation/usa.xlsx", sheetName = "bothtwonpis")
hatyci<-read.xlsx("Source Data/plot_validation/usa.xlsx", sheetName = "val")

usaobserved<-Observed%>% filter(time>=201140&time<=202001)%>%
  mutate(positive_rate = replace(positive_rate, time > 201952, NA))%>%
  mutate(scenario = "Observed", mean = positive_rate,lower = positive_rate,upper = positive_rate)
tmp<-usaobserved
tmp[tmp$time>=201240&tmp$time<=202001,c("mean","lower","upper")]<-hatyci[hatyci$time>=201240&hatyci$time<=202001,c("mean","lower","upper")]
usafit<-tmp%>% filter(time>=201140&time<=202001)%>%
  mutate(mean = replace(mean, time < 201240, NA))%>%
  mutate(lower = replace(lower, time < 201240, NA))%>%
  mutate(upper = replace(upper, time < 201240, NA))%>%
  mutate(mean = replace(mean, time > 201952, NA))%>%
...
plot_usa_fit <-ggplot()+
  ...
  labs(x="year", y = "Percent Positive (%)") +
  scale_x_continuous(breaks=usaxlabelpos, label=usaxlabel,expand = c(0.01,0),sec.axis=dup_axis(name="",breaks = c(1,379),labels = c("","")))+
  scale_y_continuous(limits = c(0, ylimits), breaks = seq(0, ylimits, by = 0.1* scale)) +
  theme_classic() +
  theme(axis.line = element_blank()) +
  geom_segment(aes(x = 1, xend = 430, y = -Inf, yend = -Inf), color = "darkgray") +
...
title <- ggdraw()+
  draw_label("Training", hjust = 1, size=13,vjust = 2.3)+
  draw_label("Test", hjust = -23.5, size=13,vjust = 2.3)
...
prow <- plot_grid(title,
                  ...
                  label_x = c(0,0.001, 0.001, 0.001, 0.001,1), 
...
outfile <- glue("Source plot/output/Validation_2011-2019.tif")
